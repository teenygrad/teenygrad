...
/// 1-D circular padding forward pass.
///
/// Grid: `pid = (b * C + c) * num_ol_tiles + ol_tile`
///
/// For output position `op`:
/// - `ip_raw = op - PAD_LEFT`
/// - if `ip_raw < 0`: wraps to `ip_raw + L`
/// - if `ip_raw >= L`: wraps to `ip_raw - L`
/// - else: `ip_raw`
///
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let val_left = T::load(
        input_ptr.add_offsets(ip_wrapped_left + in_bc_base),
    ...
    let val_right = T::load(
        input_ptr.add_offsets(ip_wrapped_right + in_bc_base),
...
/// 1-D circular padding backward pass.
///
/// Multiple output positions may wrap to the same input position, so
/// `atomic_add` is used. `dx` must be zero-initialised before launch.
#[kernel]
pub fn circular_pad1d_backward<
    T: Triton,
    D: Num,
    const PAD_LEFT: i32,
    const PAD_RIGHT: i32,
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let ip_wrapped_left = ip_raw + L;
    let ip_wrapped_right = ip_raw - L;
    ...
    T::atomic_add(
        dx_ptr.add_offsets(ip_wrapped_left + dx_bc_base),
        dy_tile,
        Some(ol_mask & cond_left),
        None,
        None,
    ...
    T::atomic_add(
        dx_ptr.add_offsets(ip_wrapped_right + dx_bc_base),
        dy_tile,
        Some(ol_mask & cond_right),
        None,
        None,
...
pub struct CircularPad1dOp<'a, T: Num> {
