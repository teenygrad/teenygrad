...
pub fn upsample_nearest2d_forward<
    ...
    const BLOCK_OW: i32,
>(
    x_ptr: In<T::Pointer<D>>,  // input  [B, C, H, W]
    y_ptr: Out<T::Pointer<D>>, // output [B, C, OH, OW]
    _B: i32,
    C: i32,
    H: i32,
    W: i32,
...
pub fn upsample_nearest2d_backward<
    ...
    const BLOCK_IW: i32,
>(
    dy_ptr: In<T::Pointer<D>>,  // upstream grad [B, C, OH, OW]
    dx_ptr: Out<T::Pointer<D>>, // input    grad [B, C, H,  W]
    _B: i32,
    C: i32,
    H: i32,
    W: i32,
...
impl<D: Num + Send + Sync + 'static> teeny_core::model::RuntimeOp for UpsampleNearest2dForward<D> {
    ...
    fn backward_grid(&self, input_shapes: &[&[usize]], _output_shape: &[usize]) -> [u32; 3] {
        let in_shape = input_shapes[0]; // [B, C, H, W]
        let num_iw_tiles = in_shape[3].div_ceil(self.block_ow as usize);
        [
            (in_shape[0] * in_shape[1] * in_shape[2] * num_iw_tiles) as u32,
            1,
            1,
        ]
    }
