...
pub fn channel_cat_forward<T: Triton, D: Num, const BLOCK_SIZE: i32>(
    ...
    y_ptr: Out<T::Pointer<D>>, // output:    n_spatial * c_total  (wide NC)
...
pub fn channel_cat_backward<T: Triton, D: Num, const BLOCK_SIZE: i32>(
    dy_ptr: In<T::Pointer<D>>, // upstream grad: n_spatial * c_total  (wide NC)
    dx_ptr: Out<T::Pointer<D>>, // input grad:    n_spatial * chunk_c  (narrow NC)
    chunk_c: i32,
    c_total: i32,
    chunk_offset: i32,
) where
