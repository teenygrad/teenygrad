    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let a  = T::load(a_ptr.add_offsets(offsets),  Some(in_bounds), None, &[], None, None, None, false);
    let b  = T::load(b_ptr.add_offsets(offsets),  Some(in_bounds), None, &[], None, None, None, false);
    T::store(da_ptr.add_offsets(offsets), dy * b, Some(in_bounds), &[], None, None);
    T::store(db_ptr.add_offsets(offsets), dy * a, Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let in_bounds = offsets.lt(n_elements);
    let dy = T::load(dy_ptr.add_offsets(offsets), Some(in_bounds), None, &[], None, None, None, false);
    T::store(da_ptr.add_offsets(offsets),  dy, Some(in_bounds), &[], None, None);
    T::store(db_ptr.add_offsets(offsets), -dy, Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let b  = T::load(b_ptr.add_offsets(offsets),  Some(in_bounds), None, &[], None, None, None, false);
    T::store(da_ptr.add_offsets(offsets), dy / b,            Some(in_bounds), &[], None, None);
    T::store(db_ptr.add_offsets(offsets), -(a * dy / (b * b)), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    T::store(out_ptr.add_offsets(offsets), y, Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let a = T::load(a_ptr.add_offsets(offsets), Some(in_bounds), None, &[], None, None, None, false);
    let b = T::load(b_ptr.add_offsets(offsets), Some(in_bounds), None, &[], None, None, None, false);
    T::store(out_ptr.add_offsets(offsets), T::minimum(a, b), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    T::store(da_ptr.add_offsets(offsets), T::where_(a_is_min, dy, z), Some(in_bounds), &[], None, None);
    T::store(db_ptr.add_offsets(offsets), T::where_(a_is_min, z, dy), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let a = T::load(a_ptr.add_offsets(offsets), Some(in_bounds), None, &[], None, None, None, false);
    let b = T::load(b_ptr.add_offsets(offsets), Some(in_bounds), None, &[], None, None, None, false);
    T::store(out_ptr.add_offsets(offsets), T::maximum(a, b), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    T::store(da_ptr.add_offsets(offsets), T::where_(a_is_max, dy, z), Some(in_bounds), &[], None, None);
    T::store(db_ptr.add_offsets(offsets), T::where_(a_is_max, z, dy), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    T::store(out_ptr.add_offsets(offsets), (a + b) / two, Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let two = T::cast::<f32, D>(T::full::<f32>(&[BLOCK_SIZE], 2.0_f32), None, false);
    let half_dy = dy / two;
    T::store(da_ptr.add_offsets(offsets), half_dy, Some(in_bounds), &[], None, None);
    T::store(db_ptr.add_offsets(offsets), half_dy, Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    T::store(out_ptr.add_offsets(offsets), T::where_(cond, one, zero), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    let offsets = T::arange(0, BLOCK_SIZE) + block_start;
    let in_bounds = offsets.lt(n_elements);
    let cond = T::load(cond_ptr.add_offsets(offsets), Some(in_bounds), None, &[], None, None, None, false);
    ...
    T::store(out_ptr.add_offsets(offsets), T::where_(bool_cond, x, y), Some(in_bounds), &[], None, None);
}
    ...
    T::Pointer<D>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<D>>>,
{
    ...
    T::store(dx_ptr.add_offsets(offsets),    T::where_(bool_cond, dy, zero), Some(in_bounds), &[], None, None);
    T::store(dy_in_ptr.add_offsets(offsets), T::where_(bool_cond, zero, dy), Some(in_bounds), &[], None, None);
}
