...
/// ```
///
/// `step_size = lr * rect / bias_correction1` is precomputed on host, where
/// `rect = sqrt((rho_t-4)(rho_t-2)*rho_inf / ((rho_inf-4)(rho_inf-2)*rho_t))`.
///
/// Grid: `[ceil(n_elements / BLOCK_SIZE), 1, 1]`.
#[kernel]
pub fn radam_adaptive_step<T: Triton, const BLOCK_SIZE: i32>(
    ...
    T::Pointer<f32>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<f32>>>,
{
    ...
    let step_t      = T::full(&[BLOCK_SIZE], step_size);
...
/// ```text
/// exp_avg    = beta1 * exp_avg    + (1 - beta1) * g
/// exp_avg_sq = beta2 * exp_avg_sq + (1 - beta2) * g²   (state updated but unused)
/// p         -= step_size * exp_avg
/// ```
///
/// `step_size = lr / bias_correction1` is precomputed on host.
///
/// Grid: `[ceil(n_elements / BLOCK_SIZE), 1, 1]`.
#[kernel]
    ...
    T::Pointer<f32>: AddOffsets<i32, 1, T::I32Tensor, Output = T::Tensor<T::Pointer<f32>>>,
{
    ...
    let exp_avg_new    = beta1_t * exp_avg    + one_m_beta1 * g_eff;
    let exp_avg_sq_new = beta2_t * exp_avg_sq + one_m_beta2 * g_eff * g_eff;
    let p_new          = p - step_t * exp_avg_new;
