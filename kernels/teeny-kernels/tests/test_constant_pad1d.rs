...
fn test_constant_pad1d_forward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad1d::ConstantPad1dForward::<
...
fn test_constant_pad1d_backward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad1d::ConstantPad1dBackward::<
        f32, PAD_LEFT, PAD_RIGHT, BLOCK_OL,
    >::new();
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
...
#[test]
#[cfg(feature = "cuda")]
fn test_constant_pad1d_forward_cuda() -> Result<()> {
    dotenv()?;
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad1d::ConstantPad1dForward<f32, PAD_LEFT, PAD_RIGHT, BLOCK_OL>,
    >(&ptx)?;
    ...
    let args = (
        ...
        B as i32, C as i32, L as i32, OL as i32, VALUE,
    );
    ...
    for i in 0..(B * C * OL) {
        assert!((output_host[i] - expected[i]).abs() < 1e-5,
            "constant_pad1d_forward mismatch at {i}: gpu={}, expected={}", output_host[i], expected[i]);
    }
    Ok(())
}
...
#[test]
#[cfg(feature = "cuda")]
fn test_constant_pad1d_backward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad1d::ConstantPad1dBackward<f32, PAD_LEFT, PAD_RIGHT, BLOCK_OL>,
    >(&ptx)?;
    ...
    let args = (
        ...
        B as i32, C as i32, L as i32, OL as i32,
    );
    device.launch(&program, &cfg, args)?;
    ...
    for i in 0..(B * C * L) {
        assert!((dx_host[i] - expected[i]).abs() < 1e-5,
            "constant_pad1d_backward mismatch at {i}: gpu={}, expected={}", dx_host[i], expected[i]);
    }
    Ok(())
}
