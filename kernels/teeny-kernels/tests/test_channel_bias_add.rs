...
fn test_channel_bias_add_forward_cuda() -> Result<()> {
    ...
    let cfg = CudaLaunchConfig { grid: [C as u32, 1, 1], block: [BLOCK_N as u32, 1, 1], cluster: [1,1,1] };
    ...
    for i in 0..N_ELEM {
        assert!(
            ...
            "forward mismatch at {i}: gpu={} expected={}", y_host[i], expected[i]
        );
    }
    Ok(())
}
...
fn test_channel_bias_add_backward_cuda() -> Result<()> {
    ...
    for i in 0..N_ELEM {
        assert!((dx_host[i] - expected_dx[i]).abs() < 1e-5, "dx mismatch at {i}: gpu={} expected={}", dx_host[i], expected_dx[i]);
    }
    for c in 0..C {
        assert!((dbias_host[c] - expected_dbias[c]).abs() < 1e-3, "dbias mismatch at {c}: gpu={} expected={}", dbias_host[c], expected_dbias[c]);
    }
    Ok(())
}
