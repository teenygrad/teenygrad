...
fn test_relu() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::relu::ReluForward::<f32>::new(1024);
...
fn test_relu_forward_gpu() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::relu::ReluForward::<f32>::new(BLOCK_SIZE);
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::relu::ReluForward<f32>,
    >(&ptx)?;
...
fn test_relu_backward_gpu() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::relu::ReluBackward::<f32>::new(BLOCK_SIZE);
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;

    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::relu::ReluBackward<f32>,
    >(&ptx)?;
