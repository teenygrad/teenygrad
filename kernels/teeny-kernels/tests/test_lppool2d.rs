...
#[test]
fn test_lppool2d_forward_asm() -> anyhow::Result<()> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!("lppool2d_forward_asm_{}", teeny_runtime::BACKEND_NAME),
        asm
    );
...
#[test]
fn test_lppool2d_backward_asm() -> anyhow::Result<()> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!("lppool2d_backward_asm_{}", teeny_runtime::BACKEND_NAME),
        asm
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_lppool2d_forward() -> anyhow::Result<()> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "hardware")]
fn test_lppool2d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;
