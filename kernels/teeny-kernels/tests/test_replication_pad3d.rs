...
#[test]
fn test_replication_pad3d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    let kernel = teeny_kernels::nn::pad::replication_pad3d::ReplicationPad3dForward::<f32>::new(
    ...
    assert_debug_snapshot!(
        format!(
            "replication_pad3d_forward_source_{}",
    ...
    assert_debug_snapshot!(
        format!(
            "replication_pad3d_forward_asm_{}",
            teeny_runtime::BACKEND_NAME
        ),
        asm
    );
...
#[test]
fn test_replication_pad3d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    let kernel = teeny_kernels::nn::pad::replication_pad3d::ReplicationPad3dBackward::<f32>::new(
        PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW,
    );
    let target = teeny_runtime::reference_target();
    let ptx_path = PathBuf::from(teeny_runtime::compile_kernel(
    ...
    assert_debug_snapshot!(
        format!(
            "replication_pad3d_backward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!(
            "replication_pad3d_backward_asm_{}",
            teeny_runtime::BACKEND_NAME
        ),
        asm
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_replication_pad3d_forward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "replication_pad3d/x.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "replication_pad3d/expected_forward.bin",
    );
    ...
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::replication_pad3d::ReplicationPad3dForward<f32>,
    >(&ptx_path)?;
    ...
    for i in 0..(B * C * OD * OH * OW) {
        assert!(
            ...
            "replication_pad3d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "hardware")]
fn test_replication_pad3d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "replication_pad3d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "replication_pad3d/expected_backward.bin",
    );
    ...
    let ptx_path = teeny_runtime::compile_kernel(&kernel, &target, true, false)?;
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::replication_pad3d::ReplicationPad3dBackward<f32>,
    >(&ptx_path)?;
    ...
    for i in 0..(B * C * DV * H * W) {
        assert!(
            ...
            "replication_pad3d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
