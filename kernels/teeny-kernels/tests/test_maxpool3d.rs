...
#[test]
fn test_maxpool3d_forward_mlir_output() -> anyhow::Result<()> {
    dotenv().ok();

    let kernel = teeny_kernels::nn::pool::maxpool3d::Maxpool3dForward::<f32>::new(
    ...
    assert_debug_snapshot!(
        format!("maxpool3d_forward_source_{}", teeny_runtime::BACKEND_NAME),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("maxpool3d_forward_mlir_{}", teeny_runtime::BACKEND_NAME),
        mlir.trim()
    );
...
#[test]
fn test_maxpool3d_backward_mlir_output() -> anyhow::Result<()> {
    dotenv().ok();

    let kernel = teeny_kernels::nn::pool::maxpool3d::Maxpool3dBackward::<f32>::new(
        KD, KH, KW, STRIDE_D, STRIDE_H, STRIDE_W, BLOCK_OW,
    );
    let target = teeny_runtime::reference_target();
    let ptx_path = PathBuf::from(teeny_runtime::compile_kernel(
    ...
    assert_debug_snapshot!(
        format!("maxpool3d_backward_source_{}", teeny_runtime::BACKEND_NAME),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("maxpool3d_backward_mlir_{}", teeny_runtime::BACKEND_NAME),
        mlir.trim()
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_maxpool3d_forward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool3d/x.bin");
    let expected = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool3d/expected_forward.bin");
    ...
    for i in 0..(B * C * OD * OH * OW) {
        assert!(
            ...
            "maxpool3d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "hardware")]
fn test_maxpool3d_backward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let x_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool3d/x.bin");
    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool3d/dy.bin");
    let expected_fwd = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool3d/expected_forward.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "maxpool3d/expected_backward.bin",
    );
    ...
    for i in 0..(B * C * DV * H * W) {
        assert!(
            ...
            "maxpool3d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
