...
fn test_lppool1d_forward_mlir_output() -> Result<()> {
    ...
    assert_debug_snapshot!("lppool1d_forward_mlir", mlir.trim());

    Ok(())
}

#[test]
fn test_lppool1d_backward_mlir_output() -> Result<()> {
    dotenv().ok();
    ...
    assert_debug_snapshot!("lppool1d_backward_mlir", mlir.trim());
...
#[test]
#[cfg(feature = "cuda")]
fn test_lppool1d_forward_cuda() -> Result<()> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "cuda")]
fn test_lppool1d_backward_cuda() -> Result<()> {
    dotenv().ok();
    let env = testing::setup_cuda_env()?;
    let device = env.device;
