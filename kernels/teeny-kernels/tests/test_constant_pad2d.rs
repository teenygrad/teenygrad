...
fn test_constant_pad2d_forward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad2d::ConstantPad2dForward::<
...
fn test_constant_pad2d_backward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad2d::ConstantPad2dBackward::<
        f32, PT, PB, PL, PR, BLOCK_OW,
    >::new();
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
...
fn test_constant_pad2d_forward_cuda() -> Result<()> {
    ...
    let input_host = load_fixture("constant_pad2d/x.bin");
    let expected = load_fixture("constant_pad2d/expected_forward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad2d::ConstantPad2dForward<f32, PT, PB, PL, PR, BLOCK_OW>,
    >(&ptx)?;
    ...
    let args = (
        ...
        B as i32, C as i32, H as i32, W as i32, OH as i32, OW as i32, VALUE,
    );
    ...
    for i in 0..(B * C * OH * OW) {
        assert!((output_host[i] - expected[i]).abs() < 1e-5,
            "constant_pad2d_forward mismatch at {i}: gpu={}, expected={}", output_host[i], expected[i]);
    }
    Ok(())
}
...
fn test_constant_pad2d_backward_cuda() -> Result<()> {
    ...
    let dy_host = load_fixture("constant_pad2d/dy.bin");
    let expected = load_fixture("constant_pad2d/expected_backward.bin");
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad2d::ConstantPad2dBackward<f32, PT, PB, PL, PR, BLOCK_OW>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * H * W) {
        assert!((dx_host[i] - expected[i]).abs() < 1e-5,
            "constant_pad2d_backward mismatch at {i}: gpu={}, expected={}", dx_host[i], expected[i]);
    }
    Ok(())
}
