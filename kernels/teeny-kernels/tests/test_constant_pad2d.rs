...
{
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad2d::ConstantPad2dForward::<f32>::new(PT, PB, PL, PR, BLOCK_OW);
...
{
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad2d::ConstantPad2dBackward::<f32>::new(PT, PB, PL, PR, BLOCK_OW);
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_constant_pad2d_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad2d::ConstantPad2dForward<f32>,
    >(&ptx)?;
...
fn test_constant_pad2d_backward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad2d::ConstantPad2dBackward<f32>,
    >(&ptx)?;
