...
#[test]
fn test_constant_pad2d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "constant_pad2d_forward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("constant_pad2d_forward_asm_{}", teeny_runtime::BACKEND_NAME),
        asm
    );
    Ok(())
}
...
#[test]
fn test_constant_pad2d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "constant_pad2d_backward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_constant_pad2d_forward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "constant_pad2d/x.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "constant_pad2d/expected_forward.bin",
    );
    ...
    for i in 0..(B * C * OH * OW) {
        assert!(
            ...
            "constant_pad2d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "hardware")]
fn test_constant_pad2d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "constant_pad2d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "constant_pad2d/expected_backward.bin",
    );
    ...
    for i in 0..(B * C * H * W) {
        assert!(
            ...
            "constant_pad2d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
