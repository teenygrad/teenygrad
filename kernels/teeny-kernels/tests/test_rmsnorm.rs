...
fn test_rms_norm_forward_cuda() -> Result<()> {
    ...
    let rrms_buf = device.buffer::<f32>(M)?;
    ...
    for i in 0..M * N {
        assert!(
            (y_host[i] - expected_y[i]).abs() < 1e-4,
            "rms_norm_forward y mismatch at {i}: gpu={}, expected={}",
            y_host[i],
            expected_y[i]
        );
    }
    for i in 0..M {
        assert!(
            (rrms_host[i] - expected_rrms[i]).abs() < 1e-4,
            "rms_norm_forward rrms mismatch at row {i}: gpu={}, expected={}",
            rrms_host[i],
            expected_rrms[i]
        );
    }
