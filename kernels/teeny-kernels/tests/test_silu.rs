...
fn test_silu_asm() -> anyhow::Result<()> {
    dotenv().ok();
    let kernel = teeny_kernels::nn::activation::silu::SiluForward::<f32>::new(BLOCK_SIZE);
...
fn test_silu_forward() -> anyhow::Result<()> {
    ...
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::activation::silu::SiluForward<f32>,
    >(&ptx_path)?;
...
fn test_silu_backward() -> anyhow::Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::silu::SiluBackward::<f32>::new(BLOCK_SIZE);
    let ptx_path = teeny_runtime::compile_kernel(
        &kernel,
        &teeny_runtime::default_target(&device)?,
        true,
    ...
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::activation::silu::SiluBackward<f32>,
    >(&ptx_path)?;
    let cfg = teeny_runtime::launch_config(N, &program);
    device.launch(
        &program,
