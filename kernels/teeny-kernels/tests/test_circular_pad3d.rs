...
{
    dotenv()?;
    let kernel = teeny_kernels::pad::circular_pad3d::CircularPad3dForward::<f32>::new(PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW);
...
{
    dotenv()?;
    let kernel = teeny_kernels::pad::circular_pad3d::CircularPad3dBackward::<f32>::new(PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW);
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_circular_pad3d_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::circular_pad3d::CircularPad3dForward<f32>,
    >(&ptx)?;
...
fn test_circular_pad3d_backward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::circular_pad3d::CircularPad3dBackward<f32>,
    >(&ptx)?;
