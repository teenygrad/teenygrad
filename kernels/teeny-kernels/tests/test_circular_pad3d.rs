...
fn test_circular_pad3d_forward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::pad::circular_pad3d::CircularPad3dForward::<f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW>::new();
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::circular_pad3d::CircularPad3dForward::<f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * OD * OH * OW) {
        assert!((output_host[i] - expected[i]).abs() < 1e-5,
            "circular_pad3d_forward mismatch at {i}: gpu={}, expected={}", output_host[i], expected[i]);
    }
    Ok(())
}
...
fn test_circular_pad3d_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::pad::circular_pad3d::CircularPad3dBackward::<f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW>::new();
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::circular_pad3d::CircularPad3dBackward::<f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * DV * H * W) {
        assert!((dx_host[i] - expected[i]).abs() < 1e-5,
            "circular_pad3d_backward mismatch at {i}: gpu={}, expected={}", dx_host[i], expected[i]);
    }
    Ok(())
}
