...
fn test_circular_pad3d_forward_cuda() -> anyhow::Result<()> {
    ...
    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "circular_pad3d/x.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "circular_pad3d/expected_forward.bin",
    );
...
fn test_circular_pad3d_backward_cuda() -> anyhow::Result<()> {
    ...
    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "circular_pad3d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "circular_pad3d/expected_backward.bin",
    );
