...
#[test]
fn test_reflection_pad2d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "reflection_pad2d_forward_source_{}",
...
#[test]
fn test_reflection_pad2d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "reflection_pad2d_backward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!(
            "reflection_pad2d_backward_asm_{}",
            teeny_runtime::BACKEND_NAME
        ),
        asm
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_reflection_pad2d_forward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "reflection_pad2d/x.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "reflection_pad2d/expected_forward.bin",
    );
    ...
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::reflection_pad2d::ReflectionPad2dForward<f32>,
    >(&ptx_path)?;
    ...
    for i in 0..(B * C * OH * OW) {
        assert!(
            ...
            "reflection_pad2d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "hardware")]
fn test_reflection_pad2d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "reflection_pad2d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "reflection_pad2d/expected_backward.bin",
    );
    ...
    let ptx_path = teeny_runtime::compile_kernel(&kernel, &target, true, false)?;
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::reflection_pad2d::ReflectionPad2dBackward<f32>,
    >(&ptx_path)?;
    ...
    for i in 0..(B * C * H * W) {
        assert!(
            ...
            "reflection_pad2d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
