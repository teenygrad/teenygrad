...
#[test]
fn test_avgpool3d_forward_mlir_output() -> Result<()> {
    dotenv()?;

    let kernel = teeny_kernels::nn::pool::avgpool3d::Avgpool3dForward::<f32>::new(KD, KH, KW, STRIDE_D, STRIDE_H, STRIDE_W, BLOCK_OW);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("avgpool3d_forward_source", kernel.source());
    assert_debug_snapshot!("avgpool3d_forward_mlir", mlir.trim());

    Ok(())
}

#[test]
fn test_avgpool3d_backward_mlir_output() -> Result<()> {
    dotenv()?;

    let kernel = teeny_kernels::nn::pool::avgpool3d::Avgpool3dBackward::<f32>::new(KD, KH, KW, STRIDE_D, STRIDE_H, STRIDE_W, BLOCK_OW);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("avgpool3d_backward_source", kernel.source());
    assert_debug_snapshot!("avgpool3d_backward_mlir", mlir.trim());
...
#[test]
#[cfg(feature = "cuda")]
fn test_avgpool3d_forward_cuda() -> Result<()> {
    dotenv()?;
    ...
    println!("[avgpool3d_forward] compiled PTX: {ptx_path}");
    let ptx = std::fs::read(&ptx_path)?;

    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::pool::avgpool3d::Avgpool3dForward<f32>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * OD * OH * OW) {
        assert!(
            ...
            "avgpool3d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "cuda")]
fn test_avgpool3d_backward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;
    ...
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    println!("[avgpool3d_backward] compiled PTX: {ptx_path}");
    let ptx = std::fs::read(&ptx_path)?;

    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::pool::avgpool3d::Avgpool3dBackward<f32>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * DV * H * W) {
        assert!(
            ...
            "avgpool3d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
