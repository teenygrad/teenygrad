...
#[test]
fn test_avgpool3d_forward_mlir_output() -> anyhow::Result<()> {
    dotenv().ok();

    let kernel = teeny_kernels::nn::pool::avgpool3d::Avgpool3dForward::<f32>::new(
    ...
    assert_debug_snapshot!(
        format!("avgpool3d_forward_source_{}", teeny_runtime::BACKEND_NAME),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("avgpool3d_forward_mlir_{}", teeny_runtime::BACKEND_NAME),
        mlir.trim()
    );
...
#[test]
fn test_avgpool3d_backward_mlir_output() -> anyhow::Result<()> {
    dotenv().ok();

    let kernel = teeny_kernels::nn::pool::avgpool3d::Avgpool3dBackward::<f32>::new(
        KD, KH, KW, STRIDE_D, STRIDE_H, STRIDE_W, BLOCK_OW,
    );
    let target = teeny_runtime::reference_target();
    let ptx_path = PathBuf::from(teeny_runtime::compile_kernel(
    ...
    assert_debug_snapshot!(
        format!("avgpool3d_backward_source_{}", teeny_runtime::BACKEND_NAME),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("avgpool3d_backward_mlir_{}", teeny_runtime::BACKEND_NAME),
        mlir.trim()
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_avgpool3d_forward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "hardware")]
fn test_avgpool3d_backward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;
