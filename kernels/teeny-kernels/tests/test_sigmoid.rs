...
fn test_sigmoid_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::activation::sigmoid::SigmoidForward::new(BLOCK_SIZE);
...
fn test_silu_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::activation::sigmoid::SiluForward::new(BLOCK_SIZE);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_logsigmoid_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::activation::sigmoid::LogsigmoidForward::new(BLOCK_SIZE);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_sigmoid_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::sigmoid::SigmoidForward
    >(&ptx)?;
...
fn test_sigmoid_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::sigmoid::SigmoidBackward::new(BLOCK_SIZE);
    let ptx_path = compile_kernel(&kernel, &Target::new(env.capability), true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::sigmoid::SigmoidBackward
    >(&ptx)?;
    let cfg = testing::launch_config(N, BLOCK_SIZE);
...
fn test_silu_forward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::sigmoid::SiluForward
    >(&ptx)?;
    let cfg = testing::launch_config(N, BLOCK_SIZE);
    env.device.launch(&program, &cfg, (x_buf.as_device_ptr() as *mut f32, y_buf.as_device_ptr() as *mut f32, N as i32))?;
    y_buf.to_host(&mut y_host)?;
...
fn test_silu_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::sigmoid::SiluBackward::new(BLOCK_SIZE);
    let ptx_path = compile_kernel(&kernel, &Target::new(env.capability), true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::sigmoid::SiluBackward
    >(&ptx)?;
    let cfg = testing::launch_config(N, BLOCK_SIZE);
    env.device.launch(&program, &cfg, (
        dy_buf.as_device_ptr() as *mut f32,
...
fn test_logsigmoid_forward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::sigmoid::LogsigmoidForward
    >(&ptx)?;
    let cfg = testing::launch_config(N, BLOCK_SIZE);
    env.device.launch(&program, &cfg, (x_buf.as_device_ptr() as *mut f32, y_buf.as_device_ptr() as *mut f32, N as i32))?;
    y_buf.to_host(&mut y_host)?;
...
fn test_logsigmoid_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::sigmoid::LogsigmoidBackward::new(BLOCK_SIZE);
    let ptx_path = compile_kernel(&kernel, &Target::new(env.capability), true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::sigmoid::LogsigmoidBackward
    >(&ptx)?;
    let cfg = testing::launch_config(N, BLOCK_SIZE);
    env.device.launch(&program, &cfg, (
        dy_buf.as_device_ptr() as *mut f32,
