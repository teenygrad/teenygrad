...
fn test_constant_pad3d_forward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad3d::ConstantPad3dForward::<
    ...
    assert_debug_snapshot!("constant_pad3d_forward_source", kernel.source());
    assert_debug_snapshot!("constant_pad3d_forward_mlir", mlir.trim());
    Ok(())
}
...
fn test_constant_pad3d_backward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::constant_pad3d::ConstantPad3dBackward::<
        f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW,
    >::new();
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
    assert_debug_snapshot!("constant_pad3d_backward_source", kernel.source());
    assert_debug_snapshot!("constant_pad3d_backward_mlir", mlir.trim());
    Ok(())
}

#[test]
#[cfg(feature = "cuda")]
fn test_constant_pad3d_forward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;

    let input_host = load_fixture("constant_pad3d/x.bin");
    let expected = load_fixture("constant_pad3d/expected_forward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad3d::ConstantPad3dForward<f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW>,
    >(&ptx)?;
    ...
    let args = (
        ...
        B as i32, C as i32, DV as i32, H as i32, W as i32, OD as i32, OH as i32, OW as i32, VALUE,
    );
    ...
    for i in 0..(B * C * OD * OH * OW) {
        assert!((output_host[i] - expected[i]).abs() < 1e-5,
            "constant_pad3d_forward mismatch at {i}: gpu={}, expected={}", output_host[i], expected[i]);
    }
    Ok(())
}
...
#[test]
#[cfg(feature = "cuda")]
fn test_constant_pad3d_backward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;

    let dy_host = load_fixture("constant_pad3d/dy.bin");
    let expected = load_fixture("constant_pad3d/expected_backward.bin");
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::constant_pad3d::ConstantPad3dBackward<f32, PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW>,
    >(&ptx)?;
    ...
    let args = (
        ...
        B as i32, C as i32, DV as i32, H as i32, W as i32, OD as i32, OH as i32, OW as i32,
    );
    device.launch(&program, &cfg, args)?;
    ...
    for i in 0..(B * C * DV * H * W) {
        assert!((dx_host[i] - expected[i]).abs() < 1e-5,
            "constant_pad3d_backward mismatch at {i}: gpu={}, expected={}", dx_host[i], expected[i]);
    }
    Ok(())
}
