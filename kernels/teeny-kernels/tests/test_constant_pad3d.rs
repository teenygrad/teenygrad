...
#[test]
fn test_constant_pad3d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!("constant_pad3d_forward_asm_{}", teeny_runtime::BACKEND_NAME),
        asm
    );
    Ok(())
}
...
#[test]
fn test_constant_pad3d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "hardware")]
fn test_constant_pad3d_forward() -> anyhow::Result<()> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "hardware")]
fn test_constant_pad3d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;
