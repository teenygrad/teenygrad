...
fn test_elemwise_add_forward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            ...
            "forward mismatch at {i}: gpu={} expected={}", out_host[i], expected[i]
        );
    }
    Ok(())
}
...
fn test_elemwise_add_backward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            ...
            "grad_a mismatch at {i}: gpu={} expected={}", ga_host[i], expected_ga[i]
        );
        assert!(
            (gb_host[i] - expected_gb[i]).abs() < 1e-6,
            "grad_b mismatch at {i}: gpu={} expected={}", gb_host[i], expected_gb[i]
        );
    }
    Ok(())
}
