...
fn test_leaky_relu_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::nn::activation::misc::LeakyReluForward::new(BLOCK_SIZE);
...
fn test_softsign_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::nn::activation::misc::SoftsignForward::new(BLOCK_SIZE);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_softplus_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::nn::activation::misc::SoftplusForward::new(BLOCK_SIZE);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_leaky_relu_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::LeakyReluBackward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
...
fn test_threshold_forward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::ThresholdForward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
...
fn test_threshold_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::ThresholdBackward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
...
fn test_softsign_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::SoftsignBackward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
...
fn test_softshrink_forward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::SoftshrinkForward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
...
fn test_softshrink_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::SoftshrinkBackward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
...
fn test_softplus_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::activation::misc::SoftplusBackward::new(BLOCK_SIZE);
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
