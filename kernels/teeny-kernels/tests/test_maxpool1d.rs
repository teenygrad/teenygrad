...
#[test]
fn test_maxpool1d_forward_mlir_output() -> anyhow::Result<()> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!("maxpool1d_forward_source_{}", teeny_runtime::BACKEND_NAME),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("maxpool1d_forward_mlir_{}", teeny_runtime::BACKEND_NAME),
        mlir.trim()
    );
...
#[test]
fn test_maxpool1d_backward_mlir_output() -> anyhow::Result<()> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!("maxpool1d_backward_source_{}", teeny_runtime::BACKEND_NAME),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("maxpool1d_backward_mlir_{}", teeny_runtime::BACKEND_NAME),
        mlir.trim()
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_maxpool1d_forward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool1d/x.bin");
    let expected = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool1d/expected_forward.bin");
    ...
    for i in 0..(B * C * OL) {
        assert!(
            ...
            "maxpool1d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "hardware")]
fn test_maxpool1d_backward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let x_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool1d/x.bin");
    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool1d/dy.bin");
    let expected_fwd = load_fixture(env!("CARGO_MANIFEST_DIR"), "maxpool1d/expected_forward.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "maxpool1d/expected_backward.bin",
    );
    ...
    for i in 0..(B * C * L) {
        assert!(
            ...
            "maxpool1d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
