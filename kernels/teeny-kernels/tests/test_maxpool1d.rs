...
fn test_maxpool1d_forward_mlir_output() -> Result<()> {
    ...
    assert_debug_snapshot!("maxpool1d_forward_source", kernel.source());
    assert_debug_snapshot!("maxpool1d_forward_mlir", mlir.trim());
...
fn test_maxpool1d_backward_mlir_output() -> Result<()> {
    ...
    assert_debug_snapshot!("maxpool1d_backward_source", kernel.source());
    assert_debug_snapshot!("maxpool1d_backward_mlir", mlir.trim());
...
#[test]
#[cfg(feature = "cuda")]
fn test_maxpool1d_forward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;

    let input_host = load_fixture("maxpool1d/x.bin");
    let expected = load_fixture("maxpool1d/expected_forward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pool::maxpool1d::Maxpool1dForward<f32, KL, STRIDE, BLOCK_OL>,
    >(&ptx)?;
...
#[test]
#[cfg(feature = "cuda")]
fn test_maxpool1d_backward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;

    let x_host = load_fixture("maxpool1d/x.bin");
    let dy_host = load_fixture("maxpool1d/dy.bin");
    let expected_fwd = load_fixture("maxpool1d/expected_forward.bin");
    let expected = load_fixture("maxpool1d/expected_backward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pool::maxpool1d::Maxpool1dBackward<f32, KL, STRIDE, BLOCK_OL>,
    >(&ptx)?;
