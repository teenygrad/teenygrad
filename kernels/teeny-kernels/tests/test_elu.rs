...
fn test_elu_backward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            ...
            "elu_backward at {i}: got={} expected={}",
            dx_host[i],
            expected[i]
        );
    }
...
fn test_selu_forward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            (y_host[i] - expected[i]).abs() < 1e-5,
            "selu_forward at {i}: got={} expected={}",
            y_host[i],
            expected[i]
        );
    }
...
fn test_selu_backward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            (dx_host[i] - expected[i]).abs() < 1e-5,
            "selu_backward at {i}: got={} expected={}",
            dx_host[i],
            expected[i]
        );
    }
...
fn test_celu_forward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            (y_host[i] - expected[i]).abs() < 1e-5,
            "celu_forward at {i}: got={} expected={}",
            y_host[i],
            expected[i]
        );
    }
...
fn test_celu_backward_cuda() -> Result<()> {
    ...
    for i in 0..N {
        assert!(
            (dx_host[i] - expected[i]).abs() < 1e-5,
            "celu_backward at {i}: got={} expected={}",
            dx_host[i],
            expected[i]
        );
    }
