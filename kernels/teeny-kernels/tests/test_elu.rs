...
fn test_elu_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::activation::elu::EluForward::<BLOCK_SIZE>::new();
...
fn test_selu_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::activation::elu::SeluForward::<BLOCK_SIZE>::new();
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_celu_mlir() -> anyhow::Result<()> {
    dotenv()?;
    let kernel = teeny_kernels::activation::elu::CeluForward::<BLOCK_SIZE>::new();
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_elu_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::elu::EluForward<BLOCK_SIZE>
    >(&ptx)?;
...
fn test_elu_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::elu::EluBackward::<BLOCK_SIZE>::new();
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::elu::EluBackward<BLOCK_SIZE>
    >(&ptx)?;
    env.device.launch(&program, &testing::launch_config(N, BLOCK_SIZE), (
...
fn test_selu_forward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::elu::SeluForward<BLOCK_SIZE>
    >(&ptx)?;
    env.device.launch(&program, &testing::launch_config(N, BLOCK_SIZE),
...
fn test_selu_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::elu::SeluBackward::<BLOCK_SIZE>::new();
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::elu::SeluBackward<BLOCK_SIZE>
    >(&ptx)?;
    env.device.launch(&program, &testing::launch_config(N, BLOCK_SIZE), (
        dy_buf.as_device_ptr() as *mut f32, x_buf.as_device_ptr() as *mut f32,
...
fn test_celu_forward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::elu::CeluForward<BLOCK_SIZE>
    >(&ptx)?;
    env.device.launch(&program, &testing::launch_config(N, BLOCK_SIZE),
        (x_buf.as_device_ptr() as *mut f32, y_buf.as_device_ptr() as *mut f32, N as i32, 1.0_f32))?;
    y_buf.to_host(&mut y_host)?;
...
fn test_celu_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::activation::elu::CeluBackward::<BLOCK_SIZE>::new();
    let ptx = std::fs::read(compile_kernel(&kernel, &Target::new(env.capability), true)?)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::activation::elu::CeluBackward<BLOCK_SIZE>
    >(&ptx)?;
    env.device.launch(&program, &testing::launch_config(N, BLOCK_SIZE), (
        dy_buf.as_device_ptr() as *mut f32, x_buf.as_device_ptr() as *mut f32,
        dx_buf.as_device_ptr() as *mut f32, N as i32, 1.0_f32,
