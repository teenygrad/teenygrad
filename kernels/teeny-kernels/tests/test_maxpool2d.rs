...
fn test_maxpool2d_forward_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::pool::maxpool2d::Maxpool2dForward::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, BLOCK_OW);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("maxpool2d_forward_source", kernel.source());
    assert_debug_snapshot!("maxpool2d_forward_mlir", mlir.trim());
...
fn test_maxpool2d_backward_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::pool::maxpool2d::Maxpool2dBackward::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, BLOCK_OW);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("maxpool2d_backward_source", kernel.source());
    assert_debug_snapshot!("maxpool2d_backward_mlir", mlir.trim());
...
fn test_maxpool2d_forward_cuda() -> Result<()> {
    ...
    let input_host = load_fixture("maxpool2d/x.bin");
    let expected = load_fixture("maxpool2d/expected_forward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pool::maxpool2d::Maxpool2dForward<f32>,
    >(&ptx)?;
...
fn test_maxpool2d_backward_cuda() -> Result<()> {
    ...
    let x_host = load_fixture("maxpool2d/x.bin");
    let dy_host = load_fixture("maxpool2d/dy.bin");
    let expected_fwd = load_fixture("maxpool2d/expected_forward.bin");
    let expected = load_fixture("maxpool2d/expected_backward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pool::maxpool2d::Maxpool2dBackward<f32>,
    >(&ptx)?;
