...
#[test]
fn test_circular_pad2d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    let kernel = teeny_kernels::nn::pad::circular_pad2d::CircularPad2dForward::<f32>::new(
    ...
    assert_debug_snapshot!(
        format!(
            "circular_pad2d_forward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!("circular_pad2d_forward_asm_{}", teeny_runtime::BACKEND_NAME),
        asm
    );
    Ok(())
}
...
#[test]
fn test_circular_pad2d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    let kernel = teeny_kernels::nn::pad::circular_pad2d::CircularPad2dBackward::<f32>::new(
        PT, PB, PL, PR, BLOCK_OW,
    );
    let target = teeny_runtime::reference_target();
    let ptx_path = PathBuf::from(teeny_runtime::compile_kernel(
    ...
    assert_debug_snapshot!(
        format!(
            "circular_pad2d_backward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_circular_pad2d_forward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "circular_pad2d/x.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "circular_pad2d/expected_forward.bin",
    );
    ...
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::circular_pad2d::CircularPad2dForward<f32>,
    >(&ptx_path)?;
...
#[test]
#[cfg(feature = "hardware")]
fn test_circular_pad2d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "circular_pad2d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "circular_pad2d/expected_backward.bin",
    );
    ...
    let ptx_path = teeny_runtime::compile_kernel(&kernel, &target, true, false)?;
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::circular_pad2d::CircularPad2dBackward<f32>,
    >(&ptx_path)?;
