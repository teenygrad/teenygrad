...
fn test_circular_pad2d_forward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::circular_pad2d::CircularPad2dForward::<
...
fn test_circular_pad2d_backward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::circular_pad2d::CircularPad2dBackward::<
        f32, PT, PB, PL, PR, BLOCK_OW,
    >::new();
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
...
fn test_circular_pad2d_forward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::pad::circular_pad2d::CircularPad2dForward::<f32, PT, PB, PL, PR, BLOCK_OW>::new();
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::circular_pad2d::CircularPad2dForward::<f32, PT, PB, PL, PR, BLOCK_OW>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * OH * OW) {
        assert!((output_host[i] - expected[i]).abs() < 1e-5,
            "circular_pad2d_forward mismatch at {i}: gpu={}, expected={}", output_host[i], expected[i]);
    }
    Ok(())
}
...
fn test_circular_pad2d_backward_cuda() -> Result<()> {
    ...
    let kernel = teeny_kernels::pad::circular_pad2d::CircularPad2dBackward::<f32, PT, PB, PL, PR, BLOCK_OW>::new();
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::circular_pad2d::CircularPad2dBackward::<f32, PT, PB, PL, PR, BLOCK_OW>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * H * W) {
        assert!((dx_host[i] - expected[i]).abs() < 1e-5,
            "circular_pad2d_backward mismatch at {i}: gpu={}, expected={}", dx_host[i], expected[i]);
    }
    Ok(())
}
