...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::replication_pad2d::ReplicationPad2dForward::<f32>::new(PT, PB, PL, PR, BLOCK_OW);
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::pad::replication_pad2d::ReplicationPad2dBackward::<f32>::new(PT, PB, PL, PR, BLOCK_OW);
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_replication_pad2d_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::replication_pad2d::ReplicationPad2dForward<f32>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * OH * OW) {
        assert!(
            ...
            "replication_pad2d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
fn test_replication_pad2d_backward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pad::replication_pad2d::ReplicationPad2dBackward<f32>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * H * W) {
        assert!(
            ...
            "replication_pad2d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
