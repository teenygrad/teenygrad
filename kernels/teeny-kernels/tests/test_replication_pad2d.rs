...
#[test]
fn test_replication_pad2d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "replication_pad2d_forward_source_{}",
    ...
    assert_debug_snapshot!(
        format!(
            "replication_pad2d_forward_asm_{}",
            teeny_runtime::BACKEND_NAME
        ),
        asm
    );
...
#[test]
fn test_replication_pad2d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "replication_pad2d_backward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!(
            "replication_pad2d_backward_asm_{}",
            teeny_runtime::BACKEND_NAME
        ),
        asm
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_replication_pad2d_forward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "replication_pad2d/x.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "replication_pad2d/expected_forward.bin",
    );
    ...
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::replication_pad2d::ReplicationPad2dForward<f32>,
    >(&ptx_path)?;
...
#[test]
#[cfg(feature = "hardware")]
fn test_replication_pad2d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;

    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "replication_pad2d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "replication_pad2d/expected_backward.bin",
    );
    ...
    let ptx_path = teeny_runtime::compile_kernel(&kernel, &target, true, false)?;
    let program = teeny_runtime::load_program::<
        teeny_kernels::nn::pad::replication_pad2d::ReplicationPad2dBackward<f32>,
    >(&ptx_path)?;
