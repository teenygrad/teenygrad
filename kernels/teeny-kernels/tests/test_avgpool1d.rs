...
fn test_avgpool1d_forward_cuda() -> Result<()> {
    ...
    let input_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "avgpool1d/x.bin");
    let expected = load_fixture(env!("CARGO_MANIFEST_DIR"), "avgpool1d/expected_forward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::pool::avgpool1d::Avgpool1dForward<f32>,
    >(&ptx)?;
...
fn test_avgpool1d_backward_cuda() -> Result<()> {
    ...
    let dy_host = load_fixture(env!("CARGO_MANIFEST_DIR"), "avgpool1d/dy.bin");
    let expected = load_fixture(
        env!("CARGO_MANIFEST_DIR"),
        "avgpool1d/expected_backward.bin",
    );
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::pool::avgpool1d::Avgpool1dBackward<f32>,
    >(&ptx)?;
