...
#[test]
fn test_avgpool1d_forward_mlir_output() -> Result<()> {
    dotenv()?;

    let kernel =
        teeny_kernels::pool::avgpool1d::Avgpool1dForward::<f32, KL, STRIDE, BLOCK_OL>::new();
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("avgpool1d_forward_source", kernel.source());
    assert_debug_snapshot!("avgpool1d_forward_mlir", mlir.trim());

    Ok(())
}

#[test]
fn test_avgpool1d_backward_mlir_output() -> Result<()> {
    dotenv()?;

    let kernel =
        teeny_kernels::pool::avgpool1d::Avgpool1dBackward::<f32, KL, STRIDE, BLOCK_OL>::new();
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("avgpool1d_backward_source", kernel.source());
    assert_debug_snapshot!("avgpool1d_backward_mlir", mlir.trim());
...
#[test]
#[cfg(feature = "cuda")]
fn test_avgpool1d_forward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;

    let input_host = load_fixture("avgpool1d/x.bin");
    let expected = load_fixture("avgpool1d/expected_forward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pool::avgpool1d::Avgpool1dForward<f32, KL, STRIDE, BLOCK_OL>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * OL) {
        assert!(
            ...
            "avgpool1d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
#[test]
#[cfg(feature = "cuda")]
fn test_avgpool1d_backward_cuda() -> Result<()> {
    dotenv()?;
    let env = testing::setup_cuda_env()?;
    let device = env.device;

    let dy_host = load_fixture("avgpool1d/dy.bin");
    let expected = load_fixture("avgpool1d/expected_backward.bin");
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::pool::avgpool1d::Avgpool1dBackward<f32, KL, STRIDE, BLOCK_OL>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * L) {
        assert!(
            ...
            "avgpool1d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
