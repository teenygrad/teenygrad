...
const PAD_H: i32 = 0;
const PAD_W: i32 = 0;
const OH: usize = (H + 2 * PAD_H as usize - KH as usize) / STRIDE_H as usize + 1; // 6
const OW: usize = (W + 2 * PAD_W as usize - KW as usize) / STRIDE_W as usize + 1; // 6
const BLOCK_OW: i32 = 8;

// ── Same-padding constants (PAD_H=PAD_W=1, same spatial size) ────────────────
const PAD_H_P: i32 = 1;
const PAD_W_P: i32 = 1;
const OH_P: usize = (H + 2 * PAD_H_P as usize - KH as usize) / STRIDE_H as usize + 1; // 8
...
fn test_conv2d_forward_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::conv::conv2d::Conv2dForward::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, PAD_H, PAD_W, 1, BLOCK_OW);
...
fn test_conv2d_backward_dx_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::conv::conv2d::Conv2dBackwardDx::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, PAD_H, PAD_W, 1, BLOCK_OW);
    let target = Target::new(Capability::Sm89);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_conv2d_backward_dw_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::nn::conv::conv2d::Conv2dBackwardDw::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, PAD_H, PAD_W, 1, BLOCK_OW);
    let target = Target::new(Capability::Sm89);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
// ---------------------------------------------------------------------------
// CUDA integration tests — same padding (PAD_H=PAD_W=1, OH=OW=H=W=8)
// ---------------------------------------------------------------------------
...
fn test_conv2d_padded_forward_cuda() -> Result<()> {
    ...
    let x_host = load_fixture("conv2d_padded/x.bin");
    let w_host = load_fixture("conv2d_padded/w.bin");
    let expected = load_fixture("conv2d_padded/expected_forward.bin");
    let mut y_host = vec![0.0f32; B * C_OUT * OH_P * OW_P];

    let mut x_buf = device.buffer::<f32>(B * C_IN * H * W)?;
    let mut w_buf = device.buffer::<f32>(C_OUT * C_IN * KH as usize * KW as usize)?;
    let y_buf = device.buffer::<f32>(B * C_OUT * OH_P * OW_P)?;

    x_buf.to_device(&x_host)?;
    w_buf.to_device(&w_host)?;

    let kernel = teeny_kernels::nn::conv::conv2d::Conv2dForward::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, PAD_H_P, PAD_W_P, 1, BLOCK_OW);
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    println!("[conv2d_padded_forward] compiled PTX: {ptx_path}");
    let ptx = std::fs::read(&ptx_path)?;
    ...
    let grid_size = B * C_OUT * OH_P * num_ow_tiles;
    let cfg = CudaLaunchConfig {
        grid: [grid_size as u32, 1, 1],
        block: [PTX_LAUNCH_THREADS_X, 1, 1],
        cluster: [1, 1, 1],
    ...
    for i in 0..(B * C_OUT * OH_P * OW_P) {
        assert!(
            (y_host[i] - expected[i]).abs() < 1e-4,
            "conv2d_padded_forward mismatch at {i}: gpu={}, expected={}",
            y_host[i],
            expected[i]
        );
    }
...
fn test_conv2d_padded_backward_dx_cuda() -> Result<()> {
    ...
    let dy_host = load_fixture("conv2d_padded/dy.bin");
    let w_host = load_fixture("conv2d_padded/w.bin");
    let expected = load_fixture("conv2d_padded/expected_dx.bin");
    let mut dx_host = vec![0.0f32; B * C_IN * H * W];

    let mut dy_buf = device.buffer::<f32>(B * C_OUT * OH_P * OW_P)?;
    let mut w_buf = device.buffer::<f32>(C_OUT * C_IN * KH as usize * KW as usize)?;
    let dx_buf = device.buffer::<f32>(B * C_IN * H * W)?;
    ...
    let kernel = teeny_kernels::nn::conv::conv2d::Conv2dBackwardDx::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, PAD_H_P, PAD_W_P, 1, BLOCK_OW);
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;
    ...
    for i in 0..(B * C_IN * H * W) {
        assert!(
            (dx_host[i] - expected[i]).abs() < 1e-4,
            "conv2d_padded_backward_dx mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
...
fn test_conv2d_padded_backward_dw_cuda() -> Result<()> {
    ...
    let x_host = load_fixture("conv2d_padded/x.bin");
    let dy_host = load_fixture("conv2d_padded/dy.bin");
    let expected = load_fixture("conv2d_padded/expected_dw.bin");
    let mut dw_host = vec![0.0f32; C_OUT * C_IN * KH as usize * KW as usize];
    ...
    let kernel = teeny_kernels::nn::conv::conv2d::Conv2dBackwardDw::<f32>::new(KH, KW, STRIDE_H, STRIDE_W, PAD_H_P, PAD_W_P, 1, BLOCK_OW);
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    let ptx = std::fs::read(&ptx_path)?;
    ...
    for i in 0..(C_OUT * C_IN * KH as usize * KW as usize) {
        assert!(
            (dw_host[i] - expected[i]).abs() < 1e-3,
            "conv2d_padded_backward_dw mismatch at {i}: gpu={}, expected={}",
            dw_host[i],
            expected[i]
        );
    }
