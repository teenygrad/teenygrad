...
{
    dotenv()?;
    let kernel = teeny_kernels::nn::pad::reflection_pad3d::ReflectionPad3dForward::<f32>::new(PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW);
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv()?;
    let kernel = teeny_kernels::nn::pad::reflection_pad3d::ReflectionPad3dBackward::<f32>::new(PD1, PD2, PH1, PH2, PW1, PW2, BLOCK_OW);
    let target = Target::new(teeny_cuda::compiler::target::Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_reflection_pad3d_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::pad::reflection_pad3d::ReflectionPad3dForward<f32>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * OD * OH * OW) {
        assert!(
            ...
            "reflection_pad3d_forward mismatch at {i}: gpu={}, expected={}",
            output_host[i],
            expected[i]
        );
    }
...
fn test_reflection_pad3d_backward_cuda() -> Result<()> {
    ...
    let ptx = std::fs::read(&ptx_path)?;
    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::pad::reflection_pad3d::ReflectionPad3dBackward<f32>,
    >(&ptx)?;
    ...
    for i in 0..(B * C * DV * H * W) {
        assert!(
            ...
            "reflection_pad3d_backward mismatch at {i}: gpu={}, expected={}",
            dx_host[i],
            expected[i]
        );
    }
