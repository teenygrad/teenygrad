...
fn test_reflection_pad1d_forward_mlir_output() -> std::result::Result<(), Box<dyn std::error::Error>>
{
    ...
    assert_debug_snapshot!(
        format!(
            "reflection_pad1d_forward_mlir_{}",
            teeny_runtime::BACKEND_NAME
        ),
        mlir.trim()
    );
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    assert_debug_snapshot!(
        format!(
            "reflection_pad1d_backward_mlir_{}",
            teeny_runtime::BACKEND_NAME
        ),
        mlir.trim()
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_reflection_pad1d_forward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "hardware")]
fn test_reflection_pad1d_backward_cuda() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;
