...
#[test]
fn test_reflection_pad1d_forward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "reflection_pad1d_forward_source_{}",
...
#[test]
fn test_reflection_pad1d_backward_asm() -> std::result::Result<(), Box<dyn std::error::Error>> {
    dotenv().ok();
    ...
    assert_debug_snapshot!(
        format!(
            "reflection_pad1d_backward_source_{}",
            teeny_runtime::BACKEND_NAME
        ),
        kernel.source()
    );
    assert_debug_snapshot!(
        format!(
            "reflection_pad1d_backward_asm_{}",
            teeny_runtime::BACKEND_NAME
        ),
        asm
    );
...
#[test]
#[cfg(feature = "hardware")]
fn test_reflection_pad1d_forward() -> anyhow::Result<()> {
    dotenv().ok();
...
#[test]
#[cfg(feature = "hardware")]
fn test_reflection_pad1d_backward() -> anyhow::Result<()> {
    dotenv().ok();
    let device = teeny_runtime::open()?;
