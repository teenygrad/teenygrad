...
fn build_tiled_graph() -> Graph {
    ...
    let conv_id = input.graph.borrow_mut().add_node(
        Op::Conv2d {
            ...
            kernel_h: KH as usize, kernel_w: KW as usize,
            stride_h: STRIDE_H as usize, stride_w: STRIDE_W as usize,
            padding_h: PAD_H as usize, padding_w: PAD_W as usize,
...
fn test_tiled_lowering_produces_tiled_kernel() {
    ...
    assert!(
        ...
        "expected tiled kernel name, got: {}", fused.value.name()
    );
}
...
fn test_conv2d_bn_silu_tiled_matches_reference() -> Result<()> {
    ...
    let w_host: Vec<f32> = (0..C_OUT * C_IN * KH as usize * KW as usize).map(|i| (i as f32 % 13.0 - 6.0) * 0.05).collect();
    ...
    let conv_out = conv2d_reference(&x_host, &w_host, NB, C_IN, C_OUT, HH, WW, KH as usize, KW as usize, STRIDE_H as usize, STRIDE_W as usize, PAD_H as usize, PAD_W as usize, OH, OW);
    ...
    device.launch(&program, &cfg, (
        ...
        NB as i32, C_IN as i32, C_OUT as i32,
        HH as i32, WW as i32, OH as i32, OW as i32,
        y_col_stride as i32,
    ))?;
    ...
    for bi in 0..NB {
        for co in 0..C_OUT {
            for ohi in 0..OH {
                for owi in 0..OW {
                    let gpu_val = y_gpu_flat[(bi * C_OUT + co) * OH * y_col_stride + ohi * y_col_stride + owi];
                    let ref_idx = ((bi * C_OUT + co) * OH + ohi) * OW + owi;
                    assert!(
                        (gpu_val - expected[ref_idx]).abs() < 1e-4,
                        "mismatch at [{},{},{},{}]: gpu={:.6} ref={:.6}",
                        bi, co, ohi, owi, gpu_val, expected[ref_idx],
                    );
                }
            }
        }
