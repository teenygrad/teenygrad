...
fn test_conv1d_forward_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::conv::conv1d::Conv1dForward::<f32>::new(KL, STRIDE, BLOCK_OL);
...
fn test_conv1d_backward_dx_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::conv::conv1d::Conv1dBackwardDx::<f32>::new(KL, STRIDE, BLOCK_OL);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_conv1d_backward_dw_mlir_output() -> Result<()> {
    ...
    let kernel = teeny_kernels::conv::conv1d::Conv1dBackwardDw::<f32>::new(KL, STRIDE, BLOCK_OL);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;
...
fn test_conv1d_forward_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::conv::conv1d::Conv1dForward<f32>,
    >(&ptx)?;
...
fn test_conv1d_backward_dx_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::conv::conv1d::Conv1dBackwardDx<f32>,
    >(&ptx)?;
...
fn test_conv1d_backward_dw_cuda() -> Result<()> {
    ...
    let program = testing::load_program_from_ptx::<
        teeny_kernels::conv::conv1d::Conv1dBackwardDw<f32>,
    >(&ptx)?;
