...
// ── MLIR snapshot test — PSA FA2 backward ────────────────────────────────────

#[test]
fn test_psa_fa2_backward_snapshot() -> Result<()> {
    dotenv().ok();

    let kernel = teeny_kernels::nn::attention::psa::PsaFa2Backward::new(HEAD_DIM);
    let target = Target::new(Capability::Sm90);
    let ptx_path = PathBuf::from(compile_kernel(&kernel, &target, true)?);
    let mlir = std::fs::read_to_string(ptx_path.with_extension("mlir"))?;

    assert_debug_snapshot!("psa_fa2_backward_source", kernel.source());
    assert_debug_snapshot!("psa_fa2_backward_mlir",   mlir.trim());

    Ok(())
}

// ── CUDA integration test — PSA FA2 backward ─────────────────────────────────
//
// Validates psa_fa2_backward against the existing FA2 expected fixtures.
// For a single FA2 call with zero-initialised gradient buffers, atomic_add is
// equivalent to a plain store, so dQ / dK / dV must match expected_backward_*.

#[test]
#[cfg(feature = "cuda")]
fn test_psa_fa2_backward_cuda() -> Result<()> {
    dotenv().ok();
    let env = testing::setup_cuda_env()?;
    let device = env.device;
    ...
    let k_host      = load_fixture("flash_attn2/k.bin");
    let v_host      = load_fixture("flash_attn2/v.bin");
    let o_host      = load_fixture("flash_attn2/expected_forward_o.bin");
    let l_host      = load_fixture("flash_attn2/expected_forward_l.bin");
    let do_host     = load_fixture("flash_attn2/do.bin");
    let expected_dq = load_fixture("flash_attn2/expected_backward_dq.bin");
    let expected_dk = load_fixture("flash_attn2/expected_backward_dk.bin");
    let expected_dv = load_fixture("flash_attn2/expected_backward_dv.bin");
    ...
    let mut do_buf = device.buffer::<f32>(N_QKV)?;
    let mut l_buf  = device.buffer::<f32>(N_L)?;
    // dq / dk are written via atomic_add; buffers must start at zero.
    let dq_buf = device.buffer::<f32>(N_QKV)?;
    let dk_buf = device.buffer::<f32>(N_QKV)?;
    let dv_buf = device.buffer::<f32>(N_QKV)?;
    ...
    k_buf.to_device(&k_host)?;
    v_buf.to_device(&v_host)?;
    o_buf.to_device(&o_host)?;
    do_buf.to_device(&do_host)?;
    l_buf.to_device(&l_host)?;

    let kernel = teeny_kernels::nn::attention::psa::PsaFa2Backward::new(HEAD_DIM);
    let target = Target::new(env.capability);
    let ptx_path = compile_kernel(&kernel, &target, true)?;
    println!("[psa_fa2_backward] compiled PTX: {ptx_path}");
    let ptx = std::fs::read(&ptx_path)?;

    let program = testing::load_program_from_ptx::<
        teeny_kernels::nn::attention::psa::PsaFa2Backward,
    >(&ptx)?;
    ...
    device.launch(&program, &cfg, (
        q_buf.as_device_ptr() as *mut f32,   // q_ptr
        k_buf.as_device_ptr() as *mut f32,   // k_ptr
        v_buf.as_device_ptr() as *mut f32,   // v_ptr
        o_buf.as_device_ptr() as *mut f32,   // o_ptr
        do_buf.as_device_ptr() as *mut f32,  // do_ptr
        l_buf.as_device_ptr() as *mut f32,   // l_ptr
        dq_buf.as_device_ptr() as *mut f32,  // dq_ptr (atomic_add, init=0)
        dk_buf.as_device_ptr() as *mut f32,  // dk_ptr (atomic_add, init=0)
        dv_buf.as_device_ptr() as *mut f32,  // dv_ptr (store)
        N_CTX as i32,                        // N
        SOFTMAX_SCALE,
    ))?;
    ...
    for i in 0..N_QKV {
        assert!(
            (dq_host[i] - expected_dq[i]).abs() < 1e-3,
            "psa_fa2_backward dQ mismatch at {i}: gpu={}, expected={}",
            dq_host[i], expected_dq[i],
        );
    }
    for i in 0..N_QKV {
        assert!(
            (dk_host[i] - expected_dk[i]).abs() < 1e-3,
            "psa_fa2_backward dK mismatch at {i}: gpu={}, expected={}",
            dk_host[i], expected_dk[i],
        );
    }
    for i in 0..N_QKV {
        assert!(
            (dv_host[i] - expected_dv[i]).abs() < 1e-3,
            "psa_fa2_backward dV mismatch at {i}: gpu={}, expected={}",
            dv_host[i], expected_dv[i],
        );
    }
