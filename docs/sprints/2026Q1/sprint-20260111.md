...
1. Complete type inference (for static shapes) for the basic kernel.
2. Convert ops.
   1. Conger get_program_id.
   2. Convert arange op to tt.make_range.
   3. Convert load op to tt.load.
   4. Convert store op to tt.store.
   5. Convert "less than operation" (uses arith dialect but with a tensor parameter).
   6. Convert "add offsets".
   7. Convert llvm.func to tt.func
   8. Convert llvm.return to tt.return
   9. Check all arithmetic is with the correct type.
   10. Check all unused functions are removed from the module.
   11. Convert module to PTX (sm_120)

### MVP Tasks

1. Add basic infra for running the kernel.
2. Run and test.
...
- fix the build of llvm-wrapper (duplicated link libraries), it works but I am sure the double linking is superflous.
- triton API uses varargs support from python, rust doesn't support it. Investigate what is the best way to support the additional optional parameters in the triton ops.
