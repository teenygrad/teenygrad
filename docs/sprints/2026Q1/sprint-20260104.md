...
2. Plan for conversion of basic MLIR module to Triton dialect.
   ...
   4. Convert "less than operation" (uses arith dialect but with a tensor parameter).
   5. Convert llvm.func to tt.func
   6. Check all arithmetic is with the correct type.
   7. Check all unused functions are removed from the module.
   8. Convert module to PTX (sm_120)
