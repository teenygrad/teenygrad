...
1. Get triton 3.5.1 working. ✅
2. Get the rust API to the point where the basic kernel can be written with the new API.
3. Basic compilation step to LLVM MLIR.
...
No, the triton API is more tricky to implement in rust than I thought.

- Good
  - Better understanding of the triton API requirements and what needs to be done in rust.

- Bad
  - Triton type system is too permissive, and not fit for purpose as defined.
