...
Yes, basic Rust function converted to Triton IR and the compiled to PTX using all the triton passes.

- Good
  - Basic architecture of the MLIR triton passes completed, and conversion of selected ops.

- Bad
  - C++ productivity extremely low.
  - Conversion of some operations may be tricky as Triton dialect makes heavy use of union types. We shall see if we can capture the trait info as attributes with rust.
