...
1. Convert llvm.func to tt.func.
2. Convert tl::arange op to tt.make_range.
3. Convert tl::load op to tt.load.
4. Convert tl::store op to tt.store.
5. Convert "convert i32 addition" to triton (requires promoting i32 to i64).
6. Convert "convert i32 multiplication" to triton (requires promoting i32 to i64).
7. Convert "less than operation" (uses arith dialect but with a tensor parameter).
8.  Full tensor addition kernel compiled to PTX.
9.  Add basic infra for running the kernel.
12. Run and test.
