# Week ending - 21/Dec/2025

## Sprint Goal

- Get the basic vector addition kernel compiling.

### Sprint Tasks

1. Basic compilation step to LLVM MLIR. ✅
2. Plan for conversion of basic MLIR module to Triton dialect.

### MVP Tasks

1. Convert tl::arange op to tt.make_range.
2. Convert tl::load op to tt.load.
3. Convert tl::store op to tt.store.
4. Convert "convert i32 addition" to triton (requires promoting i32 to i64).
5. Convert "convert i32 multiplication" to triton (requires promoting i32 to i64).
6. Convert "less than operation" (uses arith dialect but with a tensor parameter).
7. Full tensor addition kernel compiled to PTX.
8. Add basic infra for running the kernel.
9. Run and test.
...
Pretty much, the basic rust code using the new API compiles to MLIR. The result is now much more sophisticated than the previous approach which just used function calls, but couldn't implement different triton backends.

The next sprint will be where the rubber meets the road, the actual conversion to the triton dialect. Then it should in theory be downhill to get a working PTX.

- Good
  - The overall approach works, and can be used for other backends (e.g. cuda-tile, which I plan to do after triton).

- Bad
  - Not exactly bad, but had to use more rust features which are not stable.
