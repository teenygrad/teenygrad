...
1. Get the rust API to the point where the basic kernel can be written with the new API. ✅
2. Basic compilation step to LLVM MLIR.
...
No, the full compilation flow through the MLIR compiler did not get completed.

The real challenge will be next step, taking all this into LLVM in a way that can use the triton MLIR dialect.

- Good
  - Rust API works and is a good foundation for type safe triton implementation.
  - Got a much better understanding of Rust, traits and it's type system.
